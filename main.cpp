...
// TF Mini Plus LiDAR state variables
int lidar1_dist = 0;
int lidar2_dist = 0;

// Instantiate HardwareSerials for LiDARs
// Lidar 1 uses RX=PA10, TX=PA9 (USART1)
// Lidar 2 uses RX=PC11, TX=PC10 (UART4)
HardwareSerial SerialLidar1(PA10, PA9);
HardwareSerial SerialLidar2(PC11, PC10);

void updateLidar1() {
    while (SerialLidar1.available()) {
        static uint8_t buf[9];
        static uint8_t count = 0;
        uint8_t ch = SerialLidar1.read();
        if (count == 0 && ch != 0x59) continue;
        if (count == 1 && ch != 0x59) {
            count = 0;
            continue;
        }
...
void updateLidar2() {
    while (SerialLidar2.available()) {
        static uint8_t buf[9];
        static uint8_t count = 0;
        uint8_t ch = SerialLidar2.read();
        if (count == 0 && ch != 0x59) continue;
        if (count == 1 && ch != 0x59) {
            count = 0;
            continue;
...
{
    Serial.begin(115200);
    SerialLidar1.begin(115200); // Lidar 1
    SerialLidar2.begin(115200); // Lidar 2
    delay(2000);
...
void loop()
{
    // Continuously read LiDAR sensors
    updateLidar1();
    updateLidar2();
    ...
    if (currentTime - lastLoopTime >= 10) {
        ...
        if (currentTime - lastPrint > 200) {
            ...
            if (imu_ok) {
                ...
                Serial.print(",\"lidar1\":"); Serial.print(lidar1_dist);
                Serial.print(",\"lidar2\":"); Serial.print(lidar2_dist);
                Serial.println("}");
            } else {
